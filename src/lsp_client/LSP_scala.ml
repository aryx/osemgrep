 ...
 *
 * Extract the type string from a Metals hover response and parse it
 * into AST_generic.type_.
 *
 * Scala typed metavars use ($X: TYPE) syntax — parsed by the recursive
 * descent Scala parser as TypedExpr nodes which scala_to_generic converts
 * to TypedMetavar when the identifier is a metavar name.
 *
 * Metals hover format (in --lsp mode): MarkupContent with markdown
 * code fences. The hover value is wrapped in ```scala ... ``` fences:
 *
 ...
 *   We extract: "Int"
 *
 * coupling: Metals hover is generated by the Scala presentation compiler
