 ...
 *
 * Extract the type string from an OmniSharp hover response and parse it
 * into AST_generic.type_.
 *
 * C# typed metavars use (TYPE $X) syntax — parsed by the tree-sitter
 * C# parser as TypedMetavar nodes (same syntax as Java).
 *
 * OmniSharp hover format (in --lsp mode): MarkupContent with markdown
 * code fences. The hover value is wrapped in ```csharp ... ``` fences:
 *
 ...
 *   We extract: "string"
 *
 * coupling: the hover content is generated by OmniSharp in:
 *   https://github.com/OmniSharp/omnisharp-roslyn/blob/master/src/OmniSharp.Roslyn.CSharp/Services/Navigation/QuickInfoProvider.cs
 *)
