 ...
 *
 * Extract the type string from an Intelephense hover response and parse it
 * into AST_generic.type_.
 *
 * PHP typed metavars use ($X : TYPE) syntax (colon-separated, like TypeScript).
 * The type is parsed via Parse_metavariable_type.ml line 25: "function foo(TYPE $x) {}".
 *
 * Intelephense hover format (in --lsp mode): MarkupContent with markdown.
 *
 ...
 *   We extract: "int" (from the return type after the closing paren)
 *
 * coupling: Intelephense hover is generated by:
