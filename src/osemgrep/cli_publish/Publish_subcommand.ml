...
let get_test_code_for_config upload_target =
  ...
  let config_test_filenames =
    Rule_tests.get_config_test_filenames ~original_config:upload_target
      ~configs:config_filenames ~original_target:upload_target
  in
  ...
  let rules, errors =
    let rules_and_origins, errors =
      Rule_fetching.rules_from_dashdash_config ~rewrite_rule_ids:true
        ~token_opt:(Some caps#token) ~registry_caching:false
            ...
            List.fold_left
              (fun fail_count config_filename ->
                ...
                let first_test_case =
                  ...
                  | first :: _ -> Some first
                in
                let caps = Auth.cap_token_and_network_and_tmp_and_readdir token caps in
